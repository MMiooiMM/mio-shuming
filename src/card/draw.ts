// 分析卡片的 Canvas 繪製（SPEC-v4 #1）：固定 1080×1350，不用任何執行期套件。
// 版面內容由 layout.ts 決定，這裡只負責排字與上色；驗證在 Playwright（jsdom 沒有 Canvas）。
//
// 配色固定用 style.css `:root` 的**淺色**變數——卡片是要傳出去的圖片，不跟觀看者的深色模式。

import type { CardLayout, CardTag } from './layout.ts';
...
function layoutPass(ctx: CanvasRenderingContext2D, layout: CardLayout, s: number, paint: boolean): number {
  ...
  const text = (str: string, x: number, baseline: number, f: string, color: string, align: CanvasTextAlign = 'left') => {
    if (!paint) return;
    ctx.font = f;
    ctx.fillStyle = color;
    ctx.textAlign = align;
...
/** 畫出卡片。會先等 `document.fonts.ready`，避免字型未載入時量錯寬度。 */
export async function drawCard(layout: CardLayout): Promise<HTMLCanvasElement> {
  ...
  roundRect(ctx, MARGIN, MARGIN, layout.width - MARGIN * 2, layout.height - MARGIN * 2, 28);
  ...
  // 底部署名（SPEC-v4 #4）
  const footerY = layout.height - MARGIN - 64;
  ctx.fillStyle = COLOR.line;
  ctx.fillRect(MARGIN + PAD, footerY - 58, layout.width - (MARGIN + PAD) * 2, 2);
  ctx.font = font(28, 500);
  ctx.fillStyle = COLOR.inkSoft;
  ctx.textAlign = 'center';
  ctx.fillText(layout.footer, layout.width / 2, footerY);
