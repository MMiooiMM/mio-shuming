...
- status: CONSULT（SPEC #25「用神五行標籤」是否涵蓋用神詳情區的喜用／忌神標籤；實作已完成且驗證全綠，未 commit，留在工作樹，備份 scratchpad `B11-wip/`）
- model: opus
- depends: B10
- spec: SPEC-v4 #22–#27
- scope:
...
- evidence: （WIP，未 commit）before（HEAD 74a770b build）新 `e2e/visual-tokens.spec.ts` 6 failed / 2 passed（radio accent-color auto、移除 bg rgb(140,47,31)、用神覆寫「木」bg 綠 rgb(47,107,63)）→ after 8 passed。npm test 314 → 318（draw.test.ts 4）；test:e2e 42 → 50；build OK。深色實測 tag--neutral 7.02:1、中性 chip 13.99:1。變異：verdictClass 中性改回 tag--flat → visual-tokens 2 failed（chip bg rgb(240,240,240)），sha1 還原一致。截圖 scratchpad `B11-before/`、`B11-after2/`（390／1280＋兩張卡）。Codex review：REQUEST_CHANGES，唯一 EVIDENCE＝`src/main.ts:214` 用神詳情「喜用」仍 tag--good（見 status 的 CONSULT）。
